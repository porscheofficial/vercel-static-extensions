...
export const buildExtension = ({extension, TMP_DIR, esbuildDefine}: { extension: string; TMP_DIR: string; esbuildDefine?: Record<string, string> }): {
...
// Create a clean extension output directory
    ...
    try {
        ...
        for (const vercelOutputItem of extensionDeploymentInstructions.vercelDeployment
            .functions) {
            ...
            if (!assetConfig) {
                console.error(`No assetConfig with id ${vercelOutputItem.id} found. Please check config.json of extension ${extension}.`);
                throw new Error();
            }
            ...
            if (!buildArtifacts) {
                console.error(`No buildArtifacts with id ${vercelOutputItem.id} found. Please check config.json of extension ${extension}.`);
                throw new Error();
            }
            ...
            for (const dependencyId of vercelOutputItem.dependenciesById) {
                ...
                if (!assetConfig) {
                    console.error(`No assetConfig with id ${dependencyId} found. Please check config.json of extension ${extension}.`);
                    throw new Error();
                }
                const buildArtifacts = buildFiles.find(
                ...
                if (!buildArtifacts) {
                    console.error(`No buildArtifacts with id ${dependencyId} found. Please check config.json of extension ${extension}.`);
                    throw new Error();
                }
                const buildArtifactsDirectory = buildArtifacts[1];
                const targetPath = path.join(
        ...
        console.error("Something went wrong while building the extension. Cleaning up the directories now.", e);
